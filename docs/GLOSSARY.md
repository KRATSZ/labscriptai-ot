...
| **Probe wells** | Flex capacitive liquid probe helper (`detect_presence`, `require_presence`, `measure_height`). Live motion requires `OPENTRONS_ENABLE_PROBE_WELLS=1` and operator sign-off. Results persist via `apply_liquid_probe_results`. |
